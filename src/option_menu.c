...
static const u8 gText_OneSwitchOn[]        = _("{COLOR GREEN}{SHADOW LIGHT_GREEN}On");
static const u8 gText_OneSwitchOff[]       = _("{COLOR GREEN}{SHADOW LIGHT_GREEN}Off");
...
static void Task_OptionMenuProcessInput_Pg2(u8 taskId)
{
    ...
    else
    {
        ...
        switch (gTasks[taskId].tMenuSelection)
        {
        case MENUITEM_SWITCHBEHAVIOR:
            previousOption = gTasks[taskId].tSwitchBehavior;
            gTasks[taskId].tSwitchBehavior = SwitchBehavior_ProcessInput(gTasks[taskId].tSwitchBehavior);
