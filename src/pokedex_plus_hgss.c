...
//For Direct access from Party Screen
...
static void Task_WaitForExitStatsScreenBackToParty(u8);
static void Task_WaitForExitSearch(u8);
static void Task_ClosePokedex(u8);
static void Task_ClosePokedexToPartyScreen(u8);
...
{ 
    ...
    gTasks[taskId].func = Task_WaitForExitStatsScreenBackToParty;
}
...
void CB2_ReturnToPartyFromPokedex(void)
{
    CB2_ReturnToPartyMenuFromSummaryScreen();
}

static void Task_ClosePokedexToPartyScreen(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        ...
        FreeWindowAndBgBuffers();
        DestroyTask(taskId);
        SetMainCallback2(CB2_ReturnToPartyFromPokedex);
        m4aMPlayVolumeControl(&gMPlayInfo_BGM, TRACKS_ALL, 0x100);
        Free(sPokedexView);
    }
}
...
static void Task_HandleInfoScreenInput(u8 taskId)
{
    ...
    if (JOY_NEW(B_BUTTON))
    {
        ...
        if(gSpeciesToLoad != SPECIES_NONE)
        {
            ...
            gTasks[taskId].func = Task_ClosePokedexToPartyScreen;
...
static void Task_WaitForExitStatsScreenBackToParty(u8 taskId)
{
        ...
        else
        {
            ...
            for (u32 j = 0; evolutions[i].params != NULL && evolutions[i].params[j].condition != CONDITIONS_END; j++)
            {
                ...
                case IF_ATK_GT_SPATK:
                    StringAppend(gStringVar4, COMPOUND_STRING("Atk > Sp-Atk"));
                    break;
